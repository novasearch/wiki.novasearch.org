...
`cd `<opencv_source>  
`mkdir build && cd "$_"`  
`cmake -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=`<install_folder>` -D INSTALL_C_EXAMPLES=OFF -D INSTALL_PYTHON_EXAMPLES=ON -D OPENCV_EXTRA_MODULES_PATH=`<opencv_contrib_dir>`/modules -D BUILD_EXAMPLES=ON -D ENABLE_FAST_MATH=1 -D CUDA_FAST_MATH=1 -D WITH_OPENCL=ON ..`  
`make -j5`

#### Adding support for FFmpeg

Assuming that FFmpeg was compiled previously and the build directory is
<ffmpeg_build>, the following environmental must be set:
...
automatically downloaded. The md5 of the downloaded file will not match
the hardcoded hash on the cmake.
...
<opencv_source>`/3rdparty/ippicv/downloads/unpack/`. Steps:
